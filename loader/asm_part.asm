...
extern _memcopy_c:near ; void _memcopy_c(uint16_t es_, uint16_t ds_, uint16_t di_, uint16_t si_, uint16_t bx_, uint16_t cx_)

GAME_START_sub_1_memcpy proc near
...
; seems to be a offset overflow safe memcpy that copies in 512 byte blocks
GAME_START_sub_1_memcpy proc near		; CODE XREF: EXE_HEADER_sub_2+8Ep
...
loc_556:				; CODE XREF: GAME_START_sub_1+47j
    ;---------- normalize ds:si pointer that offset is minimal
    mov ax, si
    shr ax, 1
    shr ax, 1
    shr ax, 1
    ...
    mov ds, dx
    and si, 0Fh
    ;---------- normalize es:di pointer that offset is minimal
    mov ax, di
    shr ax, 1
    shr ax, 1
    shr ax, 1
...
loc_557:				; CODE XREF: EXE_HEADER_sub_2+7j
    ...
    cld
  	...
  	lds	si, dword ptr cs:exe_pointer.ofs ; src_
    test  cs:byte_55, 40h
    jz  short loc_560
    jmp loc_561
; ---------------------------------------------------------------------------
...
;----------    
    ...
    rcl ax, 1
		...
		rcl	ax, 1		;
					...
					; bx ==	(result	& 0xFFFF));
;----------              
		les	di, dword ptr cs:pointer3.ofs ; dest 
    mov cx, bx ; cx = lo uint32 header_size*16 
    mov bx, ax ; bx = hi uint32 header_size*16
		call	GAME_START_sub_1_memcpy ; es,ds,di,si,cx,bx saved - ax,dx gets overwritten
    
    mov dx, bx ; dx = hi uint32 header_size*16
    mov si, cx ; si = lo uint32 header_size*16 
    mov bx, cs:word_63
    mov cx, cs:word_62
    sub cx, si
    sbb bx, dx
    mov cs:word_63, bx
    mov cs:word_62, cx
		les	di, dword ptr cs:exe_pointer.ofs ; dest_
		call	GAME_START_sub_1_memcpy ; es,ds,di,si,cx,bx saved - ax,dx gets overwritten
...
ENDIF    
		...
		jnc	short locret_654 ; if(!CF)...
		call	START_GAME_DOES_FILE_EXIST_sub_19
		jnc	short loc_655	; if(!CF)... CF	set on file exists
