...
uint16_t get_flags_value(const emu_t::flags_t& flags_)
{
	...
	const uint16_t result = 0b1111000000101010 // all reserved = 1
	                      + (flags_.carry << 0) + (flags_.parity << 2) +
	                        (flags_.adjust << 4) + (flags_.zero << 6) +
	                        (flags_.sign << 7) + (flags_.trap << 8) +
	                        (flags_.intr << 9) + (flags_.dir << 10) +
	                        (flags_.oflow << 11);
...
#define ASM_OP_2_RES(OPERATION, OP1, OP2, REG1, REG2) assert(false);
#define ASM_OP16_2_RES(OPERATION, OP1, OP2)           assert(false);
#define ASM_OP8_2_RES(OPERATION, OP1, OP2)            assert(false);
#define ASM_OP_2(OPERATION, OP1, OP2, REG1, REG2)     assert(false);
#define ASM_OP16_2(OPERATION, OP1, OP2)               assert(false);
#define ASM_OP8_2(OPERATION, OP1, OP2)                assert(false);
#define ASM_OP_1_RES(OPERATION, OP1, REG1)            assert(false);
#define ASM_OP16_1_RES(OPERATION, OP1)                assert(false);
#define ASM_OP8_1_RES(OPERATION, OP1)                 assert(false);
...
#define ASM_OP_2_RES(OPERATION, OP1, OP2, REG1, REG2) \
	{ \
		...
		{__asm pushf \
\
		 __asm push flags_value __asm popf \
\
		 __asm mov REG1, \
		 REG1##_value __asm mov REG2, \
		 OP2 __asm OPERATION REG1, \
		 REG2 \
\
		 __asm pushf __asm pop flags_value \
\
		 __asm mov REG1##_value, \
		 REG1 \
\
		 __asm popf} OP1     = REG1##_value; \
...
\
		 ...
		 REG1 \
\
		 __asm popf} /* there is no result to set */ \
		set_flags_value(flags, flags_value); \
	}
...
\
		 __asm mov REG1, \
		 REG1##_value __asm OPERATION REG1 \
\
		 __asm pushf __asm pop flags_value \
\
		 __asm mov REG1##_value, \
...
void emu_t::and(uint16_t& op1_, uint16_t op2_)
{
...
void emu_t:: xor
                (uint8_t & op1_, uint8_t op2_) { ASM_OP8_2_RES(xor, op1_, op2_) }

                void emu_t::
                or (uint16_t & op1_, uint16_t op2_)
{
	ASM_OP16_2_RES(or, op1_, op2_)
}

void emu_t:: or (uint8_t & op1_, uint8_t op2_)
{
	ASM_OP8_2_RES(or, op1_, op2_)
}

void emu_t:: xor
        (uint16_t & op1_, uint16_t op2_) { ASM_OP16_2_RES(xor, op1_, op2_) }
...
void emu_t::intr_0x21()
{
	...
	case 0x3f: // file_read
	{
		...
		// hexdump(buffer, count, 5*16).c_str());
