...
namespace
{
    ...
    uint16_t get_flags_value( const emu_t::flags_t& flags_ )
    {
        ...
        const uint16_t result = 0b1111000000101010 // all reserved = 1
                                + ( flags_.carry << 0 ) + ( flags_.parity << 2 ) + ( flags_.adjust << 4 ) +
                                ( flags_.zero << 6 ) + ( flags_.sign << 7 ) + ( flags_.trap << 8 ) +
                                ( flags_.intr << 9 ) + ( flags_.dir << 10 ) + ( flags_.oflow << 11 );
...
\
          __asm mov REG1, \
          REG1##_value __asm mov REG2, OP2 __asm OPERATION REG1, \
          REG2 \
\
